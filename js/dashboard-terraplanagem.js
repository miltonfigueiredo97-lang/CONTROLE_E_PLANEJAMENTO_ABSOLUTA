    ...
    try {
      ...
      el.innerHTML = `
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px;margin-bottom:10px;">
          <div class="db-metrica-card"><div class="db-metrica-valor">${entregas.length}${viagensTotalEstimado != null ? ` <span style="font-size:.65em;color:#888;">/ ${viagensTotalEstimado}</span>` : ''}</div><div class="db-metrica-label">🚚 Viagens atual / total</div><div class="db-metrica-sub">estimado pelo volume total ÷ capacidade média</div></div>
          ${volTotalRetirada > 0 ? `<div class="db-metrica-card"><div class="db-metrica-valor" style="color:#15803d;">${Utils.formatarNumero(volTotalRetirada, 1)} m³</div><div class="db-metrica-label">Volume total a retirar</div><div class="db-metrica-sub">terra + estacas + fundação, empolados</div></div>` : ''}
          <div class="db-metrica-card"><div class="db-metrica-valor" style="color:#c2600a;">${Utils.formatarNumero(volTerra, 1)} m³</div><div class="db-metrica-label">Volume executado</div><div class="db-metrica-sub">${pct != null ? Utils.formatarNumero(pct, 1) + '% da terraplanagem' : 'sem previsto pra comparar'}</div></div>
          ${volFaltando != null ? `<div class="db-metrica-card"><div class="db-metrica-valor">${Utils.formatarNumero(volFaltando, 1)} m³</div><div class="db-metrica-label">⏳ Volume faltando</div></div>` : ''}
          ${custoTotal > 0 ? `<div class="db-metrica-card"><div class="db-metrica-valor" style="color:#1d4ed8;">R$ ${Utils.formatarNumero(custoTotal)}</div><div class="db-metrica-label">💰 Valor gasto</div></div>` : ''}
          ${valorFaltando != null && valorFaltando > 0 ? `<div class="db-metrica-card"><div class="db-metrica-valor">R$ ${Utils.formatarNumero(valorFaltando)}</div><div class="db-metrica-label">Valor faltando (estimado)</div></div>` : ''}
        </div>
        <div style="font-size:.72rem;font-weight:700;color:var(--cor-texto-muted);text-transform:uppercase;letter-spacing:.4px;margin:2px 0 6px;">Composição</div>
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px;margin-bottom:14px;">
          <div class="db-metrica-card"><div class="db-metrica-valor" style="color:#c2600a;">${volEmpolado > 0 ? Utils.formatarNumero(volEmpolado, 1) + ' m³' : '—'}</div><div class="db-metrica-label">Volume de terra (previsto)</div><div class="db-metrica-sub">${volEmpolado > 0 ? '+' + Utils.formatarNumero((Number(cfgDoc?.taxaEmpolamento) || 0) * 100, 1) + '% empolamento' : 'Levantamento pendente'}</div></div>
          <div class="db-metrica-card"><div class="db-metrica-valor">${volEntulho > 0 ? Utils.formatarNumero(volEntulho, 1) + ' m³' : '—'}</div><div class="db-metrica-label">🧱 Volume de entulho</div><div class="db-metrica-sub">não entra na terraplanagem</div></div>
          <div class="db-metrica-card"><div class="db-metrica-valor">${volEstacasEmpolado > 0 ? Utils.formatarNumero(volEstacasEmpolado, 1) + ' m³' : '—'}</div><div class="db-metrica-label">Volume fundação profunda</div><div class="db-metrica-sub">${volEstacasEmpolado > 0 ? '+' + Utils.formatarNumero((Number(cfgDoc?.taxaEmpolamento) || 0) * 100, 1) + '% empolamento' : 'Controle de Estacas'}</div></div>
          <div class="db-metrica-card"><div class="db-metrica-valor">${volFundSupEmpolado > 0 ? Utils.formatarNumero(volFundSupEmpolado, 1) + ' m³' : '—'}</div><div class="db-metrica-label">Volume fundação superficial</div><div class="db-metrica-sub">Controle de Fundações</div></div>
        </div>
