...
const Producao = (() => {
  ...
  function editarQtd(id) {
    if(!Permissions.pode('producao','editar:quantidade')){Utils.toast('Sem permissão para editar.','erro');return;}
  ...
  function editarEquipe(id) {
    if(!Permissions.pode('producao','editar:equipe')){Utils.toast('Sem permissão para editar.','erro');return;}
    const t = tarefas.find(x => x.id === id); if (!t) return;
