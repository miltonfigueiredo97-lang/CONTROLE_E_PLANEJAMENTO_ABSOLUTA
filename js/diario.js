...
const Diario = (() => {
  ...
  async function pautaSalvarAvanco(tid){
    if(!Permissions.pode('diario','criar:pautaRapida')){Utils.toast('Sem permissão.','erro');return;}
    const t=tarefas.find(x=>x.id===tid);if(!t)return;
    const inp=document.getElementById('pt-perc-'+tid);
    const v=parseFloat(inp?.value);
  ...
  async function avulsaAdd(){
    if(!Permissions.pode('diario','criar:avulsa')){Utils.toast('Sem permissão.','erro');return;}
  ...
  async function avulsaExcluir(id){
    if(!Permissions.pode('diario','excluir:avulsa')){Utils.toast('Sem permissão.','erro');return;}
  ...
  function _pautaHtml(){
    ...
    <div class="pt-grupo">
      ...
      <div class="pt-card" style="display:flex;gap:8px;">
        ...
        <button class="btn btn-sm btn-primario" title="Adicionar tarefa avulsa (fora do planejamento) — rola para os próximos dias até concluir" data-perm="diario:criar:avulsa" onclick="Diario.avulsaAdd()">＋ Adicionar</button>
      </div>
      ${avPend.map(a=>`<div class="pt-card"><div class="pt-l1">
        ...
        <button class="btn-icone" title="Excluir" data-perm="diario:excluir:avulsa" onclick="Diario.avulsaExcluir('${a.id}')">🗑️</button>
      </div></div>`).join('')}
    ...
    <div class="dia-form">
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-top:12px;">
        ...
        <div style="display:flex;gap:8px;">
          ...
          <button class="btn btn-sm btn-primario" data-perm="diario:criar:lancamento" onclick="Diario.salvar()">${_editId?'💾 Salvar alteração':'＋ Lançar'}</button>
        </div>
      </div>
    </div>`:''}
      ...
      Object.entries(D_STATUS).map(([k,v])=>{
        ...
        return `<div class="dia-sec-t" style="color:${v.cor};">${v.label} <span style="color:#94a3b8;font-weight:600;">(${ls.length})</span></div>`+
          ls.map(l=>`<div class="dia-lanc">
            ...
            <button class="btn-icone" data-perm="diario:editar:lancamento" title="Editar" onclick="Diario.editar('${l.id}')">✏️</button>
            <button class="btn-icone" data-perm="diario:excluir:lancamento" title="Excluir" onclick="Diario.excluir('${l.id}')">🗑️</button>
          </div>`).join('');
      }).join('')}`;
  }
  ...
  async function salvar(){
    if(!Permissions.pode('diario',_editId?'editar:lancamento':'criar:lancamento')){Utils.toast('Sem permissão.','erro');return;}
  ...
  function editar(id){
    if(!Permissions.pode('diario','editar:lancamento')){Utils.toast('Sem permissão para editar.','erro');return;}
  ...
  async function excluir(id){
    if(!Permissions.pode('diario','excluir:lancamento')){Utils.toast('Sem permissão para excluir.','erro');return;}
